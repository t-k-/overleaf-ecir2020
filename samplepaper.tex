...
In Figure~\ref{intro}, when indexing ``bc + xy + a + z,'' its expression ID (or ExpID) will be appended to posting lists associated with tokenized prefix paths from its OPT representation.
The shared structures highlighted in black and gray are found by matching tokenized paths, i.e.,  VAR/TIMES, VAR/ADD and VAR/TIMES/ADD starting at the same root node (i.e., ADD and TIMES on the right side).
%
Two paths match if and only if they have the same tokenized paths,
for example, ``a/+'' and ``z/+'' can be matched.

In addition to ExpID, the posting list entry stores the root node ID for indexed paths, in order to reconstruct matches substructures at merge time.
At query time, the similarity score is given by the size of matched common subtrees.
%assuming that subtrees are identical only if their leaf-root paths match.
...
%and assigning them different score weights $\alpha$ and $\beta$ (we use notation $|\cdot|$ to indicate the number of elements):
\begin{align}
\label{eq:1}
...
\end{align}
%
%According to previous experiments in \cite{a0_2019}, 
Interestingly, while multiple subtree matching boosts effectiveness, using just the widest match still outperforms other systems in terms of high relevance/similarity.
%
Scoring from the widest common subtree simplifies scoring: the resulting score for query and document OPTs $T_q, T_d$ is the widest match $w^*_{Q, D}$, formally
\begin{align}
\label{eq:2}
...
%
In Figure~\ref{intro}, the value of $w^*_{Q, D}$ is 3, for the widest common subtrees shown in gray.

\vspace{0.1in}
\noindent\textbf{Dynamic Pruning}\; In dynamic pruning, the top K scored hits are observed throughout the querying process, with the lowest score in the top K at a given point defining the threshold $\theta$. Since at most K candidates will be returned, dynamic pruning strategies estimate score upperbounds before knowing the precise score of a hit.
%
Candidate hits with a score less or equal to $\theta$ can be pruned safely, because they will not appear in the final top K results.
...
%
Our approach is different from the original MaxScore, as upperbound scores are also calculated from the query tree representation. In our approach each node has a maximum number of leaves to match.
...
To merge posting lists, a set of query paths match paths from a document expression one at a time, each time a \textit{hit path set} for matched query and candidate paths are examined.
%
...
Figure~\ref{bipart} shows the hit path set graph of the example in Figure~\ref{intro}, this example can be partitioned into independent subgraphs associated with tokenized paths VAR/TIMES/ADD, VAR/TIMES and VAR/ADD.
...
Let $G^{(m, n)} = G(Q^{(m)}, D^{(n)}, E^{(m, n)})$ be the subgraph matching between query node $n$ and document node $m$  where
$$
\begin{aligned}
...
\end{aligned}
$$
Because we only need to know the maximum matched subtree width, we can calculate the score of node pairs independently, summing to obtain the maximum matching size among partitions ($w^*_{Q, D}$).
%and the maximum matching pair of nodes is essentially our interested similarity score $w^*_{Q, D}$.
Specifically,
%
...
It is analogous to text search where each posting list has a partial score upperbound, the TF-IDF score upperbound is merely their sum. In our case, we have to sum and only select the maximum.
...
Instead, we greedily follow one branch of the binary programming sub-problems to obtain a greedy solution in $O(|T_q| |\mathfrak{T}|^2)$.
...
where each reference is associated with $w_{m,t} = |Q^{(m)}_t|$ identified by tokenized path $t$.
%
...
Each posting list corresponds to a token path $t \in \mathfrak{T}$ with a dynamic counter for the number of query nodes referring to it (initially $|Q_t|$).
...
% 
For ease of testing termination, we append ExpID \emph{MaxID} at the end of each posting list, which is larger than any ExpID in the collection.

Each posting list entry identified by an ExpID stores $n$ and $w_{n,t} = |D^{(n)}_t|$ values (denoted as $n/w_n$ of $t$) of subtree token path $t$ rooted at $n$.
%
As an example, in Figure~\ref{figillu}, the hit OPT shown at right (of ExpID 12) has 5 paths tokenized as $t =$ VAR/TIMES/ADD, 2 rooted at $d4$ and 3 rooted at $d1$. The information ($d1/3, d4/2$) is stored with  corresponding posing list $t$.
...
Then the algorithm selects the best matched query node and its matched width (\emph{widest}) is our structural similarity $w^*_{Q, D}$.
...
%as the final score for ranking which further considers symbolic similarity (e.g., to differentiate $E=mc^2$ and $y=ax^2$) and also penalizes oversized candidate document.
...
There are 20 queries containing wildcards (using wildcard specifier \textbf{\textbackslash{}qvar} to match arbitrary subexpression or symbols, e.g., query ``\textbackslash qvar\{a\} + \textbackslash qvar\{b\}'' can match ``$x^2 + (y + 1)^3$'').
...
\caption{Average run times for NTCIR-12 Wiki Formula Browsing Task (K = 1000).
}
\label{boxplot}
\end{center}
...
All systems are evaluated on the same machine\footnote{Environment: Intel Core i5 @ 3.60GHz per core, 16 GB memory and SSD drive.} in a single thread for top-1000 results.
%
...
For space, we only include the faster base-opd-only baseline.
