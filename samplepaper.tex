...
At query processing, the shared structures highlighted in black and gray are found by matching these tokenized paths (two paths match if and only if they have the same tokenized paths, for example, ``a/+'' and ``z/+'' can be matched) and common subtrees are  found by grouping paths by their root nodes.
%
...
%
The simplified similarity score based on only widest common subtree between query and document OPTs $T_q, T_d$ is the widest match $w^*_{Q, D}$, formally
\begin{align}
\label{eq:2}
...
For space, we only include the faster base-opd-only baseline into Figure~\ref{boxplot}.
