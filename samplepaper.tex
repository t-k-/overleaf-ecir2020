...
%
%Unlike standard text search, substructure similarity requires graph matching and comparison to be effective. Unfortunately, this
%requires more computation than scoring documents by common keywords and their frequencies.
%
...
%in this case, we need to capture that the scaling factor and exponent are inverted.
...
%And most of the time, only a few query terms contribute to similarity score during post list merging, which implies a good potential for pruning due to the large number of unnecessary reads operation in a long query.
...
% in substructure searching on an inverted index, simply accumulating term score upperbounds independently may produce a very loose overall upperbound (preventing us applying traditional dynamically pruning effectively),
%ntcir12
%because in structure matching, the frequency of single term occurrence does not necessarily indicate any relevance,
...
%when document tree structure information are lost after indexing.
%
